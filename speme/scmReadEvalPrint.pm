...
sub _next_token() {
  ...
  if ($str=~/^["](\\["]|[^"])*["]\s*/) {
     ...
     return new scmSExpr()->let();
  	 ...
  	 $self->{str}=~s/^defperl\s*//;
  	 return new scmSExpr()->defperl();
  } elsif ($str=~/^define([(]|\s+)/) {
     $self->{str}=~s/^define\s*//;
     ...
     $self->{str}=~s/^perl\s*//;
     return new scmSExpr()->perl("scalar");
  } elsif ($str=~/^\@perl\s+/) { 
     $self->{str}=~s/^\@perl\s*//;
     return new scmSExpr()->perl("list");   
  } elsif ($str=~/^use\s+/) {
     $self->{str}=~s/^use\s*//;
  ...
  #   return new scmSExpr()->_new();
     ...
     $self->{str}=~s/^([-@])[>]\s*//;
     my $type=($1 eq "-") ? "scalar" : "list";
     return new scmSExpr()->obj_apply($type);
...
sub check_list {
  ...
  #return "\${sub { my \$a=shift;return (ref(\$a) eq 'ARRAY') ? list(\@{\$a}) : \$a; }->(\\$res)}";
  #return "sub { my \$clid_$clid=\\$res;return (ref(\$clid_$clid) eq 'ARRAY') ? list(\@{\$clid_$clid}) : \${\$clid_$clid}; }->()";
...
sub evaluate() {
  ...
  if ($sexpr->is_list()) {
    ...
    #debug "type:".$car->type()."\n";
    if ($car->is_lambda()) {
       $res=$self->eval_lambda(@l);	# Defines a lambda, no application
       ...
       $res=$self->eval_eq_V_(@l);						# levert altijd #t of #f op
    } elsif ($car->is_case()) {
       $res=$self->eval_case(@l);
    } elsif ($car->is_and()) {
       $res=$self->eval_and(@l);						# levert altijd #t of #f op
    } elsif ($car->is_or()) {
       $res=$self->eval_or(@l);							# levert altijd #t of #f op
    } elsif ($car->is_begin()) {
       ...
       $res=$self->eval_perl($car->value(),@l);
    } elsif ($car->is_defperl()) {
       $res=$self->eval_defperl(@l);					# Rechtstreekse perl code, you're on your own
    } elsif ($car->is_compile()) {
       $res=$self->eval_compile(@l);					# Compileren is een ander soort proces
    } elsif ($car->is_use()) {
       $res=$self->eval_use(@l);						# use is een ander soort proces
    } elsif ($car->is_package()) {        # (package name version (exports) (definitions))
       $res=$self->eval_package($car,@l);				# Een package definitie is iets anders 
    } elsif ($car->is_die()) {
       $res=$self->eval_die($car,$self->scm_name(),$self->define(),@l);		# TODO - CHECK DIE
    } elsif ($car->is_obj_apply()) { 
       $res=$self->eval_obj_apply($car->value(),@l);
    } elsif ($car->is_define()) {
       $res=$self->eval_define(@l);						# Een define levert geen resultaat op.
    } elsif ($car->is_define_syntax()) {
       $res=$self->eval_define_syntax(@l);				# Idem voor define-syntax
    } elsif ($car->is_syntax_rules()) {
       $res=$self->eval_syntax_rules(@l);				# Idem voor syntax-rules
    } elsif ($car->is_operator()) {
       $res=$self->eval_operator($car,@l);
    } elsif ($car->is_identifier()) {
       # this could be syntax or a real identifier
       my $id=$self->get_identifier($car);				# Een identifier is een identifier
       debug("id=$id\n");
       ...
       if (defined($f) && (ref($f) eq "scmSyntax")) {
          $res=$f->apply_macro($self,$car,@l);			# Macro applicatie is iets anders.
       } else {
    ...
    $res=$self->eval_symbol($sexpr->value());			# Atom
  } elsif ($sexpr->is_string()) {
    $res=$self->eval_string($sexpr->value());			# Atom
  } elsif ($sexpr->is_number()) {
    $res=$self->eval_number($sexpr->value());			# Atom
  } elsif ($sexpr->is_boolean()) {
    $res=$self->eval_boolean($sexpr->value());			# Atom
  } elsif ($sexpr->is_re()) {
  	$res=$self->eval_re($sexpr->value());				# Atom
  } else {
    if ($sexpr->is_operator()) {
       $res=$self->get_operator($sexpr);				# Atom
    } elsif ($sexpr->is_identifier) {
       $res=$self->get_identifier($sexpr);				# Identifier
...
sub get_perl_func($) {
  ...
  $f=~s/^["]\s*//;
  $f=~s/\s*["]$//;
  #print "get_perl_func: f=$f\n";
  $f=~s/\\["]/"/g;		# this is real perl code. Need escaping
  $f=~s/\\\\/\\/g;
  if ($f=~/^sub[^{]*[{]/) { return $f."->("; }
  else { return $f."("; }
}
...
sub eval_perl() {
  ...
  if (scalar(@_)==0) {  
    if ($func->is_string()) {  #literal
      if ($result_type eq "list") {
	      return "list(".get_perl_func($func)."))";
	  } else {
    ...
    $res.=")";
  	if ($result_type eq "list") { return "list($res)"; }
  	else { return $res; }
  }
}
...
sub eval_defperl() {
  ...
  foreach my $expr (@_) {
  	if ($expr->is_string()) {
  		...
  		print "Warning: usage: (defperl <string> [<string> ...])\n";
  		$res="";
  		last;
  	}
  }
...
sub eval_eq_V_() {
  my $self=shift;
  my @exprs=@_;
  #debug "eq?: ".scalar(@exprs)."\n";
  my $res="perl_is_eq_V_(";
  my $comma="";
  foreach my $expr (@exprs) {
    #debug "eq?: sub expr=".$expr."\n";
    #debug "eq?: expr type:".$expr->type()."\n";
  	$res.="$comma";
  	$self->incr();
  	#debug "eq?: incr() voorbij\n";
  	#debug "is_identifier:".$expr->is_identifier()."\n";
  	#debug "is_atom:".$expr->is_atom()."\n";
  	if ($expr->is_identifier()) { 
		#debug "getting identifier\n";
  		$res.=$self->get_identifier($expr); 
  	} elsif ($expr->is_atom()) { 
  		#debug "evaluating sub expression - atom\n";
  		$res.=$self->evaluate($expr); 
  	} else { 
  		#debug "evaluating sub expression\n";
  		$res.="sub { my \$v=".$self->evaluate($expr).";if (ref(\$v)) { return \$v; } else { return \\\$v; } }->()"; 
  		#debug "res=$res\n";
  	}
 	#debug "eq?: decr()\n";
  	$self->decr();
 	#debug "eq?: decr() voorbij\n";
  	$comma=",";
  }
  $res.=")";
  return $res;
...
sub eval_set() {
  ...
  my $res="sub { ".$self->get_identifier($lval)."=".$self->evaluate($rval).";return ".$self->get_identifier($lval)."; }->()";
  $self->decr();
  return $res;
}
